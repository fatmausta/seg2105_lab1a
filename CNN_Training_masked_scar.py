...
def PatchMaker(patch_size, window_size, nclasses, pid_train, datapath, skip, scar_definition_ratio):  
    ...
    for pid in pid_train:
        ...
        for p in range(0,len(LGE_patches)):            
            ...
            LGE_windows = numpy.reshape(LGE_windows[p],(window_size*window_size))
            LGE_windows = numpy.reshape(LGE_windows, (1,window_size*window_size))    
            patch_labels_training.append(label)
            window_intensities_training.append(LGE_windows)         
        #BALANCE YOUR DATA IN A CONTROLLED WAY for each slice, slice by slice
        #1) SEPERATE SCAR FROM BACKGROUND
        for r in range(0,len(patch_labels_training)):
            if(np.sum(patch_labels_training[r])>=patchsize_sq*scar_definition_ratio):#scar 
                LGE_patches_scar.append(patch_labels_training[r])
                LGE_windows_scar.append(window_intensities_training[r])
            else: #background
                LGE_patches_bg.append(patch_labels_training[r])
                LGE_windows_bg.append(window_intensities_training[r])
        ...
        if len(LGE_patches_bg)>len(LGE_patches_scar):#drop from background data only
            ...
            controlled_datapopnumber = (desired_ratio_balance-ratio_imbalance)*len(patch_labels_training)/desired_ratio_balance                       
        ...
        patch_labels_training = np.concatenate((LGE_patches_scar,LGE_patches_bg),axis=0)
        window_intensities_training = np.concatenate((LGE_windows_scar,LGE_windows_bg),axis=0)
        LGE_patches_scar=[]
        LGE_patches_bg=[]
        LGE_windows_scar=[]
        LGE_windows_bg=[]
